...
import { DipNetworkError, redactUrl } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new DipNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${redactUrl(request.url)}`));
      return;
    }
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      ...
      timer = setTimeout(() => {
        const err = new DipNetworkError(`Request timed out after ${timeoutMs}ms`);
        fail(err);
        req.destroy(err);
